...
(async () => {
  ...
  const rawVolume = amountKRW / limitPrice;
  const volume = (Math.ceil(rawVolume * 1e8) / 1e8).toFixed(8);
  ...
  const result = {
    ...
    params: {
      ...
      rawVolume,
      volume,
      estimatedTotalKRW: Number(volume) * limitPrice,
    },
    attempts: [],
  };

  function appendLog(msg) { fs.appendFileSync(logPath, `[${new Date().toISOString()}] ${msg}\n`); }
  ...
  try {
    ...
    while (Date.now() - monitorStart <= 60000) {
      try {
        ...
        const state = String(statusResp.state || statusResp.status || '').toLowerCase();
    ...
    if (!result.status) result.status = 'submitted_unfilled_within_60s';
    result.finishedAt = new Date().toISOString();
    fs.writeFileSync(outPath, JSON.stringify(result, null, 2));
    appendLog(`Completed with status=${result.status}`);
  } catch (err) {
    result.status = 'fatal_error';
    result.error = { message: err.message, code: err.code, status: err.response?.status, data: err.response?.data };
    result.finishedAt = new Date().toISOString();
    fs.writeFileSync(outPath, JSON.stringify(result, null, 2));
